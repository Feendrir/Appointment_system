        ...
        <div class="card-body px-4 pt-4 pb-3">
          <form action="/dokter/update/<?= $dokter['id'] ?>" method="post">
            ...
            <div class="mb-4">
              ...
              <select class="form-select" id="id_poli" name="id_poli" required>
                ...
                <?php foreach ($poli as $p): ?>
                  <option value="<?= $p['id'] ?>" <?= $dokter['id_poli'] == $p['id'] ? 'selected' : '' ?>><?= esc($p['nama_poli']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            ...
            <div class="mb-4">
              ...
              <input type="text" class="form-control" id="alamat" name="alamat" value="<?= esc($dokter['alamat']) ?>" placeholder="Masukkan alamat" required>
            </div>

            <div class="d-flex justify-content-end">
              <a href="/dokter" class="btn btn-dark me-2">Batal</a>
              <button type="submit" class="btn btn-primary me-3"><i class="fas fa-save me-2"></i> Simpan Perubahan</button>
            </div>
          </form>
        </div>
      </div>
